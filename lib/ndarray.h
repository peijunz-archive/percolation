...
template <typename dtype>
void ndarray<dtype>::reset(int d, int w){
    if (dim>0){
        delete [] shape;
        delete [] stride;
        delete [] head;
...
template <typename dtype>
void ndarray<dtype>::reset(int d, int *sh){
    if (dim>0){
        delete [] shape;
        delete [] stride;
        delete [] head;
...
template <typename dtype>
ndarray<dtype>::ndarray(int d, int *sh):dim(d){
    shape=new int[d];
    stride=new int[d+1];
    stride[d]=1;
    for(int i=0;i<d;i++){
...
template <typename dtype>
ndarray<dtype>::ndarray(int d, int w):dim(d){
    shape=new int[d];
    stride=new int[d+1];
    stride[d]=1;
    for(int i=0;i<d;i++){
...
template <typename dtype>
inline int ndarray<dtype>::overflow(int index, int bound, int pn){
